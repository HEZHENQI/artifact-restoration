...
def patch_to_img(x, patch_size,num_channels=3):          #将patch转为图片
    B, N, E =x.shape
    x = x.reshape(
        B,
        int(math.sqrt(N)),
...
class SwinTransformerBlock(TimestepBlock):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.time_mlp = (
            nn.Sequential(SiLU(),nn.Linear(time_embed_dim,self.dim))
        )
    ...
    def _forward(self, x,emb):
        ...
        # partition windows
        print("shifted x shape",shifted_x.shape)
        print("window size ",self.window_size)
        nW, x_windows = window_partition(shifted_x, self.window_size)  # B, nW, window_size, window_size, C
        print("nW ",nW)
        if 1:
            ...
            print("emb size ",emb.shape)
            print(self.time_mlp)
            time_embed=self.time_mlp(emb).unsqueeze(1)
...
class BasicLayer_up(TimestepBlock):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, upsample=None, use_checkpoint=False,time_embed_dim=128*4,out_channels = 64):
...
class SwinUNetModel(nn.Module):
    def __init__(
        ...
        self.outputlayer = nn.Linear(embed_dim, self.in_channels * (patch_size ** 2))
    ...
    def forward(self, x, timesteps, y=None):
        ...
        x = self.pos_drop(x)
        # timesteps = self.time_mlp(timesteps).unsqueeze(1)
        ...
        if self.final_upsample=="expand_first":
            ...
            h = self.output(h)
        # x = self.outputlayer(h)
        # x = patch_to_img(h, self.patchsize)
