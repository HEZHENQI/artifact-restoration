...
class MiT(TimestepBlock):
    ...
    def forward(
        ...
        layer_outputs = []
        for (get_overlap_patches, overlap_embed, layers,time_layer) in self.stages:
            ...
            emb_out = time_layer(emb).type(x.dtype)
            # print('In MiT, emb after time_layer shape ',emb_out.shape)
            while len(emb_out.shape) < len(x.shape):
                emb_out = emb_out[..., None]
            # print('emb_out in MiT before add ',emb_out.shape)
            # print('x in MiT before add ',x.shape)
            x = x + emb_out
...
class Segformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        embed_dim=2*128
        model_channels = 128
        self.time_dim=4*model_channels
        self.decoder_dim = decoder_dim
        self.time_mlp = nn.Sequential(
            linear(128, self.time_dim),
            SiLU(),
...
#         ) for i, dim in enumerate(dims)])
        ...
        self.to_fused_conv = nn.ModuleList([
            nn.Conv2d(dim, decoder_dim, 1) for i, dim in enumerate(dims)])
        ...
        self.to_fused_Upsample = nn.ModuleList([
            nn.Upsample(scale_factor = 2 ** (i+2)) for i, dim in enumerate(dims)])
        ...
        # )
        self.to_segmentation = nn.Sequential(
            normalization(4 * decoder_dim),
            SiLU(),
            zero_module(nn.Conv2d(4 * decoder_dim, num_classes, 3,padding=1)),
        )
        ...
        # self.final_time_layer = nn.Sequential(
        #             SiLU(),
        #             linear(
        #                 self.time_dim,
        #                 decoder_dim
    ...
    def forward(self, x):
        ...
        emb = self.time_mlp(timestep_embedding(timestep,128))
        ...
        fused = []
        n = 0
        # for output, to_fused,time_layer in zip(layer_outputs, self.to_fused,self.time_layers):
        for output, to_fused_conv,to_fused_time_emb,to_fused_Upsample in zip(layer_outputs, self.to_fused_conv,self.to_fused_time_emb,self.to_fused_Upsample):
...
#             while len(emb_out.shape) < len(x.shape):
#                 emb_out = emb_out[..., None]
            # print('in segformer forward before add emb_out.shape ',emb_out.shape)
            # print('in segformer forward before add output.shape ',output.shape)
            # output = output + emb_out
            
            output = to_fused_conv(output)
            emb_out = to_fused_time_emb(emb)
            while len(emb_out.shape) < len(output.shape):
                emb_out = emb_out[..., None]
            output = output + emb_out
            output = to_fused_Upsample(output)
            fused.append(output)
            n+=1
        # fused = [to_fused(output) for output, to_fused in zip(layer_outputs, self.to_fused)]
        fused = torch.cat(fused, dim = 1)
        
#         final_time = self.final_time_layer(emb)
        
#         while len(final_time.shape) < len(fused.shape):
#             final_time = final_time[..., None]
#         print('final_time emb: ',final_time.shape)
#         print('fused shape: ',fused.shape)
#         fused = fused + final_time
